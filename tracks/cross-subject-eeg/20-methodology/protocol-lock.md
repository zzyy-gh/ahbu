...
defensibility-critic advisory (layers/10-pain-point-validation/
...
`30-experiments/code/labram_pretrain_datasets.txt` at environment setup,
...
`30-experiments/runs/dev_dataset_selection.txt` before any dev-split
...
`30-experiments/runs/dev_dataset_selection.txt`. The "hardware-disjoint"
...
headline run. Pass/fail logged in `30-experiments/runs/partition_audit.txt`.
...
1. A written confirmation in `30-experiments/runs/pre-run-checklist.txt`
...
`30-experiments/runs/leakage_audit_result.json`.
...
4. **Dev-dataset coverage (M-2 fix):** also call
   ...
   logged in `30-experiments/runs/leakage_audit_result.json` alongside
