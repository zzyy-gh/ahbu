...
- **Track dir:** `tracks/cross-subject-eeg/`
...
- Condition: **pre-register** the five-part evaluation program in `tracks/cross-subject-eeg/20-methodology/protocol-lock.md` before any headline experiment runs.

**From reuse sketch (methodologist a6dd95):**
- Highest-novelty shared artifact in the portfolio: `shared/eval/leakage_audit.py` (pre-training overlap audit). Promotable independently of result direction.
- Other promotion candidates: `shared/eval/fewshot_curve.py`, `shared/eval/partition.py`, `shared/models/riemannian_baseline.py`. Plausible 2nd consumers exist across affective-state, sleep-staging, ecg-ppg-realworld.
...
- [ ] Human approval to instantiate `tracks/cross-subject-eeg/`

When the human checkpoint completes, copy `tracks/_template/` → `tracks/cross-subject-eeg/`, update `portfolio.md`, tag `v2-cross-subject-eeg-admitted`.
