...
- **`shared/eval/leakage_audit.py`** — `check_subject_overlap(train_ids,
...
- **`shared/eval/fewshot_curve.py`** — `fewshot_accuracy_curve(model,
...
- **`shared/eval/partition.py`** — `subject_disjoint_split(...)` as in
...
- **`shared/models/riemannian_baseline.py`** — `fit_mdm(X_train, y_train)`,
