...
- **`shared/eval/calibration.py`** — same module as ecg-ppg-realworld sketch
...
- **`shared/eval/cohort_stratifier.py`** — `stratified_report(y_true,
...
- **`shared/eval/partition.py`** — `subject_disjoint_split(df, subject_col,
...
- **`shared/data/sleep_edf_loader.py`** — loader for Sleep-EDF-Cassette
...
- **`shared/models/hrv_feature_pipeline.py`** — `extract_hrv_features(rr_intervals,
