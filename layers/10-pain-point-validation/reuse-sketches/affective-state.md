...
- **`shared/data/hrv_eda_feature_extractor.py`** — `extract_features(signal,
  ...
  features. Note: overlaps with `shared/models/hrv_feature_pipeline.py`
  proposed by sleep-staging sketch — these should be merged into one
  canonical extractor if both tracks are admitted.

- **`shared/eval/feature_stability.py`** — `cross_dataset_correlation(
...
- **`shared/eval/partition.py`** — subject_disjoint_split as above; this
