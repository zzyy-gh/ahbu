...
- **`shared/eval/calibration.py`** — functions `brier_score(y_true, y_prob)`,
...
- **`shared/eval/abstention.py`** — `selective_classification_curve(y_true,
...
- **`shared/eval/partition.py`** — `patient_disjoint_split(df, id_col,
...
- **`shared/data/ptbxl_loader.py`** — thin wrapper around PTB-XL waveform
